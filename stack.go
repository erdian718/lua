...
func (stk *stack) bounds(seg int) (segC int, segN int) {
	...
	if seg < 0 {
		...
		if seg < 0 {
			panic("Requested frame not on the stack.")
		}
	} else {
		// Absolute index.
...
func (stk *stack) AddFrame(fn *function, fi, args, rtns int) {
	...
	if base < pbase {
		panic("Invalid argument count for AddFrame.")
	}
...
func (stk *stack) TailFrame(fn *function, fi, args int) {
	if len(stk.frames) == 0 {
		panic("No frames on the stack to use for tail call.")
	}
	frame := stk.cFrame()
	segC, segN := stk.topBounds()
...
func (stk *stack) ReturnFrame() {
	if len(stk.frames) <= 1 {
		panic("Not enough frames on the stack.")
	}
	frame := stk.cFrame()
	...
	if frame.retBase < 0 {
		panic("Frame return with invalid (missing) return data.")
	}
