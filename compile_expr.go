...
func lowerIdent(n ast.Expr, state *compState, reg int) (identData, int) {
	...
	case *ast.TableAccessor:
		...
		case *ast.FuncCall:
			...
			return *data, 1
		default:
			panic("Syntax error") // TODO: Better errors
		}
		panic("UNREACHABLE")
	case *ast.ConstIdent:
...
func (e exprData) Bool() (patchList, bool) {
	...
	default:
		return nil, toBoolean(state.f.constants[e.constant])
	}
}
...
func (e exprData) BoolReg() (patchList, bool, bool) {
	...
	default:
		state.addInst(createABx(opLoadK, e.reg, e.constant), e.line)
		return nil, true, toBoolean(state.f.constants[e.constant])
	}
}
...
func expr(e ast.Expr, state *compState, reg int, boolRev bool) exprData {
	...
	case *ast.ConstInt:
		rtn.constant = state.constK(toInteger(ee.Value))
	case *ast.ConstFloat:
